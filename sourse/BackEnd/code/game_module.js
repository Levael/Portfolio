...
function WSS_Init (http_port) {
    console.log('Socket server works on port: ' + (http_port + 1));

    wss = new ws.Server({port: http_port + 1});
    wss.on('connection', (wsc, req) => {WS_Connection(wsc, req)});
}
